...
function renderQueue() {
  ...
  $('#requestQueue').innerHTML=list.length ? list.map(g => { const r=latest(g), o=original(g[0]), f=final(g), is=orderNo(r.order_number)===activeOrder, isFinal=r.status==='approved', payment=f.diff>0, refund=f.diff<0, action=isFinal?'Bhej dein':payment?'Pehle paisay lein':refund?'Refund / credit check':r.request_type==='cancel_order'?'Order cancel check':'Abhi dispatch na karein', icon=r.source==='whatsapp'?'💬':'🛍️', moneyText=payment?`+ ${money(f.diff,f.currency)} lena hai`:refund?`${money(Math.abs(f.diff),f.currency)} wapas dena hai`:'Koi extra paisa nahi'; return `<button class="order-card ${is?'selected':''} ${isFinal?'safe':'hold'}" data-order="${orderNo(r.order_number)}"><div class="order-card-top"><span class="source ${r.source==='whatsapp'?'whatsapp-dot':'email-dot'}">${icon}</span><span><b>${r.customer_name||'Customer'} · ${orderNo(r.order_number)}</b><small>${r.source==='whatsapp'?'WhatsApp customer':'Shopify customer'} · ${g.length} request${g.length===1?'':'s'}</small></span><span class="simple-status ${isFinal?'safe':'stop'}">${isFinal?'✓ FINAL':'⏸ HOLD'}</span></div><div class="quantity-compare"><div><small>Pehle</small><b>${o.qty}</b><span>item${o.qty===1?'':'s'}</span></div><strong>→</strong><div class="latest"><small>Ab final</small><b>${f.qty}</b><span>item${f.qty===1?'':'s'}</span></div><div class="money-decision ${payment?'collect':refund?'refund':'same'}"><small>PAISON KA HISAAB</small><b>${moneyText}</b></div></div><div class="action-line"><span>${action}</span><em>Details dekhein →</em></div></button>`; }).join('') : '<div class="empty-queue">No saved requests yet. Start with Shopify order #1002.</div>';
}
